...
@interface DinUtvardering ()

@property (nonatomic, strong) CPTGraphHostingView *hostView;

@end

@implementation DinUtvardering
@synthesize datesArray,averageArray;
...
//        [plotSpace scaleToFitPlots:[NSArray arrayWithObjects:presentPlot, nil]];
        plotSpace.yRange = [CPTPlotRange plotRangeWithLocation:CPTDecimalFromFloat(0) length:CPTDecimalFromFloat(101)];
        plotSpace.xRange = [CPTPlotRange plotRangeWithLocation:CPTDecimalFromFloat(0) length:CPTDecimalFromFloat([datesArray count])];
//        CPTMutablePlotRange *xRange = [plotSpace.xRange mutableCopy];
//        [xRange expandRangeByFactor:CPTDecimalFromCGFloat(6.2f)];
//        plotSpace.xRange = xRange;
//        CPTMutablePlotRange *yRange = [plotSpace.yRange mutableCopy];
...
-(void)configureAxes {
    ...
    axisTextStyle.textAlignment = UITextAlignmentCenter;
...
//    {
    ...
    for (int k=0; k<[datesArray count]; k++) {
        NSString *dateString =[datesArray objectAtIndex:k];
        NSString *finalString = [dateString stringByReplacingOccurrencesOfString:@" (" withString:@"\n("];
        [array addObject:finalString];
    }
...
//            {
               // if ([plot.identifier isEqual:_presentDate] == YES)
                    return [NSNumber numberWithInt:[[averageArray objectAtIndex:index]intValue]];
          //  }
			break;
	}
